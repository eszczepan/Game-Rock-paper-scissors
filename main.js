//CASHED IN THE DOM - for the future use 
let userScore = 0;
let pcScore = 0;
...
function win(user, computer){
  ...
  result_div.innerHTML = `${convertChoiceToWord(user)}${smallUserWord} beats ${convertChoiceToWord(computer)}${smallPcWord} . You win!`;
...
function lose(user, computer){
  ...
  result_div.innerHTML = `${convertChoiceToWord(user)}${smallUserWord} loses to ${convertChoiceToWord(computer)}${smallPcWord}. You lost... ;(`;
...
function draw(user, computer){
  ...
  const smallPcWord = "Pc".fontsize(3).sub();
  const userChoice_div = document.getElementById(user);
  result_div.innerHTML = `${convertChoiceToWord(user)}${smallUserWord} equals ${convertChoiceToWord(computer)}${smallPcWord}. It's a draw.`;
